...
class LINUX_TexasInstruments_INA219_4_20mA(LINUX_TexasInstruments_INAGeneric):
    ...
    REG_CONFIG_VALUE_RUN = [
        LINUX_TexasInstruments_INAGeneric.REG_CONFIG, 0x1F, 0xFB]
    REG_CONFIG_VALUE_POWER_DOWN = [
        LINUX_TexasInstruments_INAGeneric.REG_CONFIG, 0x1F, 0xF8]
    ...
    def send_value_over_mqtt(self):
        ...
        time.sleep(0.5)
        # ------------------------------------------------------------------------------
        read = self.read_i2c_value(
            LINUX_TexasInstruments_INAGeneric.REG_SHUNT_VOLTAGE)
        self.print_value(
            LINUX_TexasInstruments_INAGeneric.REG_SHUNT_VOLTAGE, read)
        self.dictData['value'] = self.convert_current_data_to_unit(read)
        # ------------------------------------------------------------------------------
        read = self.read_i2c_value(
            LINUX_TexasInstruments_INAGeneric.REG_BUS_VOLTAGE)
        self.print_value(
            LINUX_TexasInstruments_INAGeneric.REG_BUS_VOLTAGE, read)
